...
function initBookingEvents(){
  ...
  $('#bookingForm').onsubmit=e=>{e.preventDefault();if(!state.time){showToast('Selecciona una hora disponible.');return} const s=services.find(x=>x.id===state.service),b=barbers.find(x=>x.id===state.barber); const appointment={id:'APT-'+Date.now(),createdAt:new Date().toISOString(),serviceId:s.id,serviceName:s.name,price:s.price,duration:s.duration,date:state.date,time:state.time,barberId:b.id,barberName:b.name,clientName:$('#clientName').value.trim(),phone:$('#clientPhone').value.trim(),email:$('#clientEmail').value.trim(),note:$('#clientNote').value.trim(),status:'confirmed'}; const arr=getAppointments();arr.push(appointment);save(STORE.appointments,arr);$('#successText').textContent=`${appointment.clientName}, tu cita quedó para ${prettyDate(appointment.date)} a las ${appointment.time} con ${appointment.barberName}.`;setStep('success');};
